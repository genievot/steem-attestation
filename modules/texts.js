...
exports.greeting = () => {
	...
	return [
		...
		`After you successfully attest your github username for the first time, `,
		`you receive a reward in Bytes that depends on your reputation in github:\n\n${rewards}\n\nHalf of the reward will be immediately available, the other half will be locked on a smart contract and can be spent after 1 year.`
	].join('');
};
...
exports.weHaveReferralProgram = (user_address) => {
	...
	return [
		"Remember, we have a referral program: you get rewards by recommending new users to link their github and Obyte accounts.  There are "+(conf.bAllowProofByPayment ? 4 : 3)+" ways to do it and ensure that the referrals are tracked to you:\n" +
...
exports.attestedFirstTimeBonus = (rewardInUSD, rewardInBytes, contractRewardInBytes, vesting_ts) => {
	...
	let text = `You attested your github username for the first time and will receive a welcome bonus of $${cashRewardInUSD.toLocaleString([], {minimumFractionDigits: 2})} (${(rewardInBytes/1e9).toLocaleString([], {maximumFractionDigits: 9})} GB) from Obyte distribution fund.`;
...
exports.referredUserBonus = (referralRewardInUSD, referralRewardInBytes, contractReferralRewardInBytes, referrer_vesting_date_ts, username) => {
	...
	let text =  `You referred user ${username} who has just verified his github username and you will receive a reward of $${cashReferralRewardInUSD.toLocaleString([], {minimumFractionDigits: 2})} (${(referralRewardInBytes/1e9).toLocaleString([], {maximumFractionDigits: 9})} GB) from Obyte distribution fund.`;
